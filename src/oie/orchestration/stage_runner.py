...
class StageRunner:
    ...
    def run_stage(self, stage_cls: Type[Stage]) -> Dict[str, Any]:
        ...
        start_index = (
            int(checkpoint["last_processed_index"]) + 1
            if checkpoint.get("last_processed_index") is not None
            else 0
        )
        ...
        try:
            ...
            failure_status = "partial_success" if checkpoint["processed_count"] > 0 else "failed"
            ...
            checkpoint["processing_time_seconds"] = round(time.perf_counter() - start_time, 6)
            self.write_checkpoint(stage, checkpoint)
