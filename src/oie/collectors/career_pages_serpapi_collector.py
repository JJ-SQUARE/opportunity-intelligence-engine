...
class CareerPagesSerpAPICollector(BaseJobCollector):
    collector_name = "career_pages_serpapi"

    def _load_legacy_callable(self):
        function_candidates = [
            ("collectors.discovery.career_pages_serpapi", "collect_jobs"),
            ("collectors.discovery.career_pages", "collect_jobs"),
            ("collectors.career_pages_serpapi", "collect_jobs"),
        ]
        ...
        class_candidates = [
            ("collectors.discovery.career_pages_serpapi", "CareerPagesSerpApiCollector"),
        ]
    ...
    def collect(self) -> List[Dict[str, Any]]:
        ...
        try:
            raw_jobs = legacy_collect_fn(
                queries=queries,
                num_pages=source_config.get("num_pages", run.get("num_pages", 3)),
                sleep_s=source_config.get("sleep_s", run.get("sleep_s", 1.0)),
            ) or []
        except TypeError:
            try:
