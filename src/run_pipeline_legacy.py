...
def run(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ok = [c for c in scored if c.get("resolved_domain")]
    print(
        f"[domain_resolution] resolved_domains={len(ok)}/{len(scored)} top5={[(c['company'], c['resolved_domain']) for c in ok[:5]]}")
        ...
        for c in scored:
            ...
            if not domain:
                ...
                if cached and not is_blocked_domain(cached):
                    ...
                    resolved = _safe_resolve_domain_serpapi(
                        c.get("company", ""),
                        enabled=serpapi_allowed,  # respeta mode=cache_only
                        serpapi_state=serpapi_state,  # respeta auto-disable 429
                    )
                    if resolved and not is_blocked_domain(resolved):
                        domain = resolved
                        set_cached_domain(c.get("company", ""), domain)
