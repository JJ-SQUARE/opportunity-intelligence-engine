...
def test_openapi_documents_run_routes():
    ...
    assert schema["paths"]["/runs/{run_id}/top-opportunities"]["get"]["summary"] == "Get run top opportunities"
    ...
    assert "RunTopOpportunitiesResponse" in schema["components"]["schemas"]
...
def test_create_run_persists_account_user_and_hubspot_delivery_metadata(tmp_path):
    ...
    assert persisted_config["runs"] == {"path": str(runs_path)}
...
def test_get_run_configuration_returns_404_when_snapshot_missing(tmp_path, monkeypatch):
    ...
    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["config_path"] = str(runs_path / ctx.run_id / "missing-configuration.json")
    write_manifest(ctx, manifest)
    ...
    client = TestClient(app)
    response = client.get(f"/runs/{ctx.run_id}/configuration")
...
def test_get_run_hubspot_delivery_returns_404_for_missing_run(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    original_create = RunContext.create
    ...
    client = TestClient(app)
    response = client.get("/runs/missing_run/hubspot-delivery")
...
def test_get_run_readiness_returns_exported_report(tmp_path, monkeypatch):
    import json

    runs_path = tmp_path / "runs"
    output_path = tmp_path / "outputs" / "run_readiness_report.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    readiness_report = {
        "ready": True,
        "warnings": [],
    ...
    output_path.write_text(json.dumps(readiness_report), encoding="utf-8")

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {
        "run_readiness_report_json": str(output_path),
    }
    write_manifest(ctx, manifest)
    ...
    client = TestClient(app)
    response = client.get(f"/runs/{ctx.run_id}/readiness")
...
def test_get_run_readiness_returns_404_when_path_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {}
    ...
    assert response.status_code == 404
    assert response.json() == {"detail": "Run readiness report not found"}


def test_get_run_readiness_returns_404_when_file_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    missing_path = tmp_path / "outputs" / "run_readiness_report.json"

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {
        "run_readiness_report_json": str(missing_path),
    }
    write_manifest(ctx, manifest)
    ...
    assert response.status_code == 404
    assert response.json() == {"detail": "Run readiness report file not found"}


def test_get_run_readiness_returns_404_for_missing_run(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    original_create = RunContext.create
    ...
    client = TestClient(app)
    response = client.get("/runs/missing_run/readiness")

    assert response.status_code == 404
    assert response.json() == {"detail": "Run not found"}


def test_get_run_analytics_returns_exported_report(tmp_path, monkeypatch):
    import json

    runs_path = tmp_path / "runs"
    output_path = tmp_path / "outputs" / "run_analytics.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    analytics = {"summary": {"status": "completed"}}
    output_path.write_text(json.dumps(analytics), encoding="utf-8")

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {"run_analytics_json": str(output_path)}
    write_manifest(ctx, manifest)
    ...
    client = TestClient(app)
    response = client.get(f"/runs/{ctx.run_id}/analytics")
...
def test_get_run_analytics_returns_404_when_path_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {}
    ...
    assert response.status_code == 404
    assert response.json() == {"detail": "Run analytics not found"}


def test_get_run_analytics_returns_404_when_file_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    missing_path = tmp_path / "outputs" / "run_analytics.json"

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {"run_analytics_json": str(missing_path)}
    write_manifest(ctx, manifest)
...
def test_get_run_top_opportunities_returns_exported_csv(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    output_path = tmp_path / "outputs" / "top_opportunities.csv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    csv_content = "company,score\nAcme,95\n"
    output_path.write_text(csv_content, encoding="utf-8")

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {"top_opportunities_csv": str(output_path)}
    write_manifest(ctx, manifest)
    ...
    client = TestClient(app)
    response = client.get(f"/runs/{ctx.run_id}/top-opportunities")

    assert response.status_code == 200
    assert response.json() == {
        "run_id": ctx.run_id,
        "path": str(output_path),
        "top_opportunities": csv_content,
    }


def test_get_run_top_opportunities_returns_404_when_path_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    ...
    assert response.status_code == 404
    assert response.json() == {"detail": "Run top opportunities not found"}


def test_get_run_top_opportunities_returns_404_when_file_missing(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    missing_path = tmp_path / "outputs" / "top_opportunities.csv"

    ctx = RunContext.create(config={"runs": {"path": str(runs_path)}})
    manifest = build_initial_manifest(ctx)
    manifest["artifact_paths"] = {"top_opportunities_csv": str(missing_path)}
    write_manifest(ctx, manifest)
    ...
    assert response.status_code == 404
    assert response.json() == {"detail": "Run top opportunities file not found"}


def test_get_run_top_opportunities_returns_404_for_missing_run(tmp_path, monkeypatch):
    runs_path = tmp_path / "runs"
    original_create = RunContext.create
    ...
    client = TestClient(app)
    response = client.get("/runs/missing_run/top-opportunities")
